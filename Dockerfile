...
RUN [ "/qemu-aarch64-static", "/bin/sh", "-c", "/qemu-aarch64-static /usr/bin/python /usr/bin/yum -y install nfs-utils epel-release jq && /qemu-aarch64-static /usr/bin/python /usr/bin/yum clean all"]
